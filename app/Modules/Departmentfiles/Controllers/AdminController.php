...
namespace Modules\Departmentfiles\Controllers;
...
class AdminController extends BaseController
{
	...
	public function __construct()
	{
		...
		$this->module = "Manage Department Files";
		$this->fp = '\Modules\Departmentfiles\Views';
		$this->default_table = DEPARTMENT_FILES_TABLE;
        $this->mr = "departmentfiles"; // Module route
    }
    ...
    public function index()
    {
	    ...
	    $parent_id = $this->common_model->SelectDropdown('tbl_department','title','id', array($this->request->getVar('parent_id')),array('status' => 1,'parent_id' => 0));
    	$page_data['parent_id'] = $parent_id;
    	$page_data['results'] = $data;
    	$page_data['module'] = 'Manage Department Files';
    ...
    public function filter_query($where = [])
    {
	  ...
	  //get employee data 
		...
		{
			$option = '<option value="">--Select Sub Department--</option>';
			if($this->request->getPost('department'))
			{
				
				$department = $this->request->getPost('department',FILTER_SANITIZE_NUMBER_INT);
				if(!empty($department)):
				  $where = 'parent_id = '.$department.' and status = 1';
				  $department = $this->common_model->SelectDropdown('tbl_department','title','id','',$where);
				  $json_data['status'] = 'success';
				  $json_data['html'] = $option.$department;
				else:
    ...
    public function add()
	{
    	...
    	if($this->request->getVar('s')):
    		...
    		$subject = $this->request->getPost('subject',FILTER_UNSAFE_RAW);
    		$type = $this->request->getPost('type',FILTER_UNSAFE_RAW);
    		$department = $this->request->getPost('department',FILTER_UNSAFE_RAW);
			$sub_department = $this->request->getPost(['sub_department'],FILTER_SANITIZE_NUMBER_INT);
			
    		$rules = [ 
    			'subject' => ['label' => 'Subject', 'rules' => 'trim|required'],
    			'type' => ['label' => 'Type', 'rules' => 'trim|required'],
    			'department' => ['label' => 'Department', 'rules' => 'trim|required'],
    			'sub_department' => ['label' => 'Sub Department', 'rules' => 'trim|required'],
    		];
    	...
    	$page_data['parent_id'] = $parent_id;
    	$page_data['module'] = 'Manage Department: Add';
    	$page_data['mr'] = $this->mr;
  ...
  public function imgUpload(){
	...
	if($this->request->getVar('s')):
		...
		$department = $this->request->getPost('department',FILTER_UNSAFE_RAW);
	    $sub_department = $this->request->getPost('sub_department',FILTER_UNSAFE_RAW);
		$subject = $this->request->getPost('subject',FILTER_UNSAFE_RAW);
		$type = $this->request->getPost('type',FILTER_SANITIZE_NUMBER_INT);
	...
	$sub_department = $this->common_model->SelectDropdown('tbl_department','title','id', array($page_data['sub_department']),array('status' => 1));
	$department_name = $this->common_model->SelectDropdown('tbl_department','title','id', array($page_data['department']),array('status' => 1,'parent_id' => 0));
	$parent_id = $this->common_model->SelectDropdown('tbl_department','title','id', array($this->request->getVar('parent_id')),array('status' => 1,'parent_id' => 0));
	$media_img = $this->common_model->GetTableRows(MEDIA_TABLE);
    
	$page_data['media_img'] = $media_img;
	$page_data['parent_id'] = $parent_id;
	$page_data['sub_department'] = $sub_department;
	$page_data['department_name'] = $department_name;
	
	$page_data['module'] = 'Manage Department: Edit';
	$page_data['mr'] = $this->mr;
	$page_data['model'] = $this->common_model;
